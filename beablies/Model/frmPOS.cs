...
namespace beablies.Model
{
    public partial class frmPOS : Form
    {
        ...
        private void BtnDessert_Click(object sender, EventArgs e)
        {
            ...
            {
                var pro = (ucProduct)item;
                pro.Visible = pro.PCategory.ToLower().Contains("dessert");
            }
        }
        ...
        private void btnKot_Click(object sender, EventArgs e)
        {
            ...
            // Periksa apakah driverID ada di tabel staff, jika driverID memiliki nilai
            if (driverID.HasValue)
            {
                string checkDriverQuery = "SELECT COUNT(*) FROM staff WHERE id = @driverID";
                MySqlCommand checkDriverCmd = new MySqlCommand(checkDriverQuery, MainClass.con);
                checkDriverCmd.Parameters.AddWithValue("@driverID", driverID.Value);

                if (MainClass.con.State == ConnectionState.Closed) { MainClass.con.Open(); }
                int driverCount = Convert.ToInt32(checkDriverCmd.ExecuteScalar());
                if (MainClass.con.State == ConnectionState.Open) { MainClass.con.Close(); }

                if (driverCount == 0)
                {
                    guna2MessageDialog1.Show("Driver ID tidak ditemukan di tabel staff.");
                    return;
                }
            }
            ...
            {
                qry1 = "UPDATE main SET date = @date, time = @time, tables = @tables, waiter = @waiter, status = @status, order_type = @order_type, total = @total, received = @received, changes = @changes, drive_id = @drive " +
                       "WHERE id = @ID";
            }
            ...
            // Atur parameter @drive sesuai nilai driverID
            if (driverID.HasValue)
            {
                cmd.Parameters.AddWithValue("@drive", driverID.Value);
            }
            else
            {
